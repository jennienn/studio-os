...
type Row={bookingId:string;customerName:string;customerPhone:string;bookingStatus:string;attendanceStatus:string|null;remaining:number|null;available:number|null};
export function AttendancePanel({studioId,timezone,role,attendanceEnabled}:{studioId:string;timezone:string;role:string;attendanceEnabled:boolean}){
 ...
 <ul className="operations-list">{rows.map(r=><li key={r.bookingId}><div>{r.customerName} · {r.customerPhone}<p>{r.remaining===null?"기간권":`잔여 ${r.remaining}회 · 예약 가능 ${r.available}회`}</p></div>
