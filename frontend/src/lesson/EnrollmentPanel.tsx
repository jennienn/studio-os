...
export function EnrollmentPanel({studioId,customerId:fixedCustomer,role,groupEnabled}:{studioId:string;customerId?:string;role:string;groupEnabled:boolean}){
 ...
 {!fixedCustomer&&<CustomerPicker studioId={studioId} label="회원" value={customer} onChange={v=>{setCustomer(v);setSelected("");setPage(0);}}/>}
 ...
 <label>수강 이력<select aria-label="수강 이력" value={selected} onChange={e=>setSelected(e.target.value)}><option value="">수강 선택</option>{items.map((e,i)=><option key={e.id} value={e.id}>{e.kind==="PRIVATE"?"개인":"그룹"} · {e.status==="ACTIVE"?"진행 중":"종료"} · {page*20+i+1}</option>)}</select></label>
 ...
 <label>구매할 이용권<select aria-label="구매할 이용권" required value={product} onChange={e=>setProduct(e.target.value)}><option value="">이용권 선택</option>{products.map(p=><option key={p.id} value={p.id}>{p.name} · {BigInt(p.price).toLocaleString()}원</option>)}</select></label>
