...
public class LessonBookingService implements BookingSpecialization {
 ...
 @Transactional public Created create(UUID studio,String key,PrivateCreate body){var actor=access.authorizedLock(studio);return idem.execute(actor,"CREATE_PRIVATE_LESSON",key,body,201,Created.class,()->{
 ...
 public void authorize(Booking b,AuthorizedStudioContext actor,String command){access.authorizedMember(b.studioId);
 ...
 public void validateTransition(Booking b,AuthorizedStudioContext actor,String command){access.lesson(b.studioId);if(command.equals("CONFIRM"))throw new ApiException(409,"ALREADY_CONFIRMED","레슨은 생성 시 확정됩니다.");}
