...
public class LessonAccess {
    ...
    public AuthorizedStudioContext member(UUID studio){
        var actor=access.member(studio);
        if(!studios.findById(actor.authorizedStudioId()).orElseThrow().businessCategory.equals("LESSON"))
            throw new ApiException(403,"LESSON_REQUIRED","레슨 사업장에서만 사용할 수 있습니다.");
        return actor;
    }
    public AuthorizedStudioContext manager(UUID studio){var actor=member(studio);if(actor.membershipRole()==StudioMembership.Role.STAFF)throw new ApiException(403,"LESSON_FORBIDDEN","레슨 관리 권한이 필요합니다.");return actor;}
