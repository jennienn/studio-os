...
public class GroupService implements BookingSpecialization {
 ...
 public record Row(UUID bookingId,UUID customerId,String customerName,String customerPhone,UUID enrollmentCycleId,String bookingStatus,String attendanceStatus,Long remaining,Long available){}
 ...
 @Transactional public Created create(UUID studio,String key,Create body){var actor=access.authorizedLock(studio);return idem.execute(actor,"CREATE_GROUP_BOOKING",key,body,201,Created.class,()->{
 ...
 @Transactional(readOnly=true) public List<Row> attendance(UUID studio,UUID occurrence,int page,int size){var actor=access.member(studio);classes.assigned(actor,classes.occurrence(studio,occurrence));PageResult.validate(page,size);
  return jdbc.query("select b.id,b.customer_id,c.name,c.phone,d.enrollment_cycle_id,b.status booking_status,a.status attendance_status from lesson_booking_details d join bookings b on b.studio_id=d.studio_id and b.id=d.booking_id join customers c on c.studio_id=b.studio_id and c.id=b.customer_id left join attendance a on a.studio_id=b.studio_id and a.booking_id=b.id where d.studio_id=? and d.class_occurrence_id=? and b.id=(select nb.id from lesson_booking_details newer join bookings nb on nb.studio_id=newer.studio_id and nb.id=newer.booking_id where newer.studio_id=d.studio_id and newer.class_occurrence_id=d.class_occurrence_id and nb.customer_id=b.customer_id order by case when exists(select 1 from attendance finalized where finalized.studio_id=nb.studio_id and finalized.booking_id=nb.id) then 0 when nb.status in ('PENDING','CONFIRMED') then 1 else 2 end,nb.created_at desc,nb.id desc limit 1) order by c.name,b.id limit ? offset ?",(r,n)->{
   ...
   return new Row(r.getObject("id",UUID.class),r.getObject("customer_id",UUID.class),r.getString("name"),r.getString("phone"),cycle,r.getString("booking_status"),r.getString("attendance_status"),bal,bal==null?null:bal-ledger.reserved(studio,cycle));
  },studio,occurrence,size,(long)page*size);
 }
 @Transactional public Result mark(UUID studio,UUID occurrence,String key,Bulk body){var actor=access.authorizedMember(studio);studios.lockById(studio).orElseThrow();classes.lockOccurrence(studio,occurrence);classes.assigned(actor,classes.occurrence(studio,occurrence));
  return idem.execute(actor,"FINALIZE_ATTENDANCE",key,Map.of("occurrence",occurrence,"body",body),200,Result.class,()->{
   access.lesson(studio);access.capability(studio,"ATTENDANCE");
   ...
   for(var entry:body.entries()){
    ...
    UUID current=jdbc.queryForObject("select b.id from lesson_booking_details d join bookings b on b.studio_id=d.studio_id and b.id=d.booking_id where d.studio_id=? and d.class_occurrence_id=? and b.customer_id=? order by case when exists(select 1 from attendance a where a.studio_id=b.studio_id and a.booking_id=b.id) then 0 when b.status in ('PENDING','CONFIRMED') then 1 else 2 end,b.created_at desc,b.id desc limit 1",UUID.class,studio,occurrence,b.customerId);
    if(!current.equals(b.id))conflict("재예약 전의 이전 예약으로 출석을 처리할 수 없습니다.");
    var previous=jdbc.query("select booking_id,status from attendance where studio_id=? and class_occurrence_id=? and customer_id=?",(r,n)->Map.entry(r.getObject("booking_id",UUID.class),r.getString("status")),studio,occurrence,b.customerId);
    if(!previous.isEmpty()){if(!previous.getFirst().getKey().equals(b.id)||!previous.getFirst().getValue().equals(entry.status().name()))conflict("확정된 출석은 변경할 수 없습니다.");continue;}
 ...
 public void authorize(Booking b,AuthorizedStudioContext actor,String command){access.authorizedManager(b.studioId);}
 public void validateTransition(Booking b,AuthorizedStudioContext actor,String command){access.lesson(b.studioId);if(command.equals("CONFIRM"))conflict("그룹 예약은 생성 시 확정됩니다.");
